...
async def upload_project_file(project_id: str, file: UploadFile = File(...), user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    filepath = os.path.join(proj_dir, safe_filename)
    async with aiofiles.open(filepath, "wb") as f:
        content = await file.read()
        await f.write(content)
    pf = ProjectFile(project_id=project_id, filename=safe_filename, filepath=filepath, file_type=file.content_type or "", file_size=len(content))
    db.add(pf)
    await db.commit()
    await db.refresh(pf)
